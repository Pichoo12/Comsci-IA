...
class User:
   ...
   def save_article(self, article: Article):
       if article.title not in [a.title for a in self.saved_articles]:
       ...
       with open(USER_DATA_FILE, 'r') as file:
           ...
           for user_data in users_data:
               user = User(
                   ...
                   saved_articles=[Article(title=article['title'], description='', content='', url='', category='General') for article in user_data.get('saved_articles', [])]
               )
...
class LoginRegisterApp(QWidget):
   ...
   Event(name="Diwali", date="2024-11-01"),
   Event(name="Hanukkah", date="2024-12-25"),
   ...
   def initUI(self):
       self.setWindowTitle("International Club")
       ...
       login_title.setStyleSheet("color: white; margin-top: 5px; margin-bottom: 5px;")
       ...
       register_title.setStyleSheet("color: white; margin-top: 10px; margin-bottom: 5px;")
   ...
   def show_article_details(self, item):
       for article in self.filtered_articles:
           if article.title == item.text():
               dialog = ArticleDetailsDialog(article, self.current_user)
               dialog.exec_()
               break
   ...
   def view_saved_articles(self):
       if self.current_user:
           saved_articles = self.current_user.saved_articles
           self.show_articles(saved_articles)
       else:
